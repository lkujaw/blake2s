...
spark_vc function_left_shift_1
  by (simp_all)
     (smt (verit) `0 <= amount` `amount <= 31`)+

spark_vc function_left_shift_4
proof -
  have "(0::int) <= 2^nat (32 - amount) - 2"
  using H1 H3 by simp
  then have "amount <= 32" using `amount <= 31`
    by simp
  then have "(value mod 2^nat (32 - amount)) * (2^nat amount) =
               value * 2^nat amount mod 2^nat 32"
    by (metis H9 mult_exp_mod_exp_eq nat_diff_distrib nat_mono)
  then have "(value AND (2^nat (32 - amount) - 1)) * (2^nat amount) =
          value * 2^nat amount mod 4294967296"
    by (subst AND_mod quadlet_pow) simp
  thus ?C1 ?C2 using H3 H6 by simp_all
qed

spark_end


(* QUADLETS/RIGHT_SHIFT *)

spark_open "quadlets/right_shift"

spark_vc function_right_shift_1
  by (simp_all)
     (smt (verit) `0 <= amount` `amount <= 31`)+
...
(* QUADLETS/RIGHT_ROTATION *)

spark_open "quadlets/right_rotation"

spark_vc function_right_rotation_3 by simp
...
(* QUADLETS/MODULAR_SUM *)

spark_open "quadlets/modular_sum"

spark_vc function_modular_sum_5
proof -
  from `negation(augend) = 4294967295 - augend`
  have "addend - negation(augend) - 1 =
          augend + addend - 4294967296" by simp
  moreover
  from `negation(augend) = 4294967295 - augend`
       `negation(augend) < addend`
  have "augend + addend >= 4294967296" by simp
  ultimately show ?thesis
    using `addend <= 4294967295`
          `negation(augend) >= 0` mod_add_if_z by simp
qed

spark_end


(* QUADLETS/MODULAR_DIFFERENCE *)

spark_open "quadlets/modular_difference"

spark_vc function_modular_difference_2
proof -
  from `minuend < subtrahend`
       `negation(subtrahend) = 4294967295 - subtrahend`
  show ?thesis by simp
qed

spark_vc function_modular_difference_4
proof -
  from `minuend >= 0`
       `subtrahend > minuend`
       `negation(subtrahend) >= 0`
       `negation(subtrahend) = 4294967295 - subtrahend`
       mod_sub_if_z
  show ?thesis by simp
qed
...
(* QUADLETS/MODULAR_PRODUCT *)

spark_open "quadlets/modular_product"

spark_vc function_modular_product_6
proof -
  have "0 <= multiplicand AND 65535" by auto
  moreover have "multiplicand AND 65535 <= 65535" by auto
  moreover
  from `0 <= multiplier`
       `multiplier <= 4294967295`
       `right_shift multiplier 16 = multiplier sdiv 65536`
       sdiv_pos_pos
  have "right_shift multiplier 16 <= 65535" by simp
  ultimately have
    "right_shift multiplier 16 *
        (multiplicand AND 65535) <= 65535^2"
      using `right_shift multiplier 16 >= 0` product_lte by blast
    thus ?thesis by auto
qed

spark_vc function_modular_product_8
proof -
  have "0 <= multiplier AND 65535" by auto
  moreover have "multiplier AND 65535 <= 65535" by auto
  moreover
  from `0 <= multiplicand`
       `multiplicand <= 4294967295`
       `right_shift multiplicand 16 = multiplicand sdiv 65536`
       sdiv_pos_pos
  have "right_shift multiplicand 16 <= 65535" by simp
  ultimately have
    "(multiplier AND 65535) *
       right_shift multiplicand 16 <= 65535^2"
      using `right_shift multiplicand 16 >= 0` product_lte by blast
    thus ?C1 ?C2 by auto
qed

spark_vc function_modular_product_12
proof -
  have "0 <= multiplier AND 65535" by simp
  moreover have "0 <= multiplicand AND 65535" by simp
  moreover have "multiplier AND 65535 <= 65535" by simp
  moreover have "multiplicand AND 65535 <= 65535" by simp
  ultimately have
    "(multiplier AND 65535) * (multiplicand AND 65535) <= 65535^2"
      using product_lte by blast
  thus ?C1 ?C2 by auto
qed

lemma safe_div: "a dvd (x::int) ==> a * (x div a * y) = x * y"
  by simp

lemma mod_div: "0 < n ==> 0 <= x ==>
    (((x::int) mod 2^n) * 2^n) mod (2^(n*2)) = (x * 2^n) mod (2^(n*2))"
  by (simp add: power2_eq_square power_mult)

lemma pow_div: "0 < n ==> 0 <= x ==> 2^n dvd y ==>
    (((x::int) mod 2^n) * y) mod (2^(n*2)) = (x * y) mod (2^(n*2))"
  by (unfold dvd_def,
      auto simp add: field_simps mod_simps power2_eq_square power_mult)

spark_vc function_modular_product_14
proof -
  have "(0::int) <= multiplier ==>
  (multiplicand mod (65536::int) * (multiplier mod (65536::int)) +
    ((65536::int) * (multiplicand div (65536::int) *
      (multiplier mod (65536::int))) +
        (65536::int) * (multiplier div (65536::int) *
          (multiplicand mod (65536::int))))
  ) mod (4294967296::int) = multiplicand * multiplier mod (4294967296::int)"
proof (induct multiplier rule: int_ge_induct)
  case base
  thus ?case by simp
next
  case (step i)
  from
    `(multiplicand mod 65536 * (i mod 65536) +
       (65536 * (multiplicand div 65536 * (i mod 65536)) +
         65536 * (i div 65536 * (multiplicand mod 65536)))) mod
           4294967296 = multiplicand * i mod 4294967296`
  have IH:
    "(multiplicand + (i mod 65536 * (multiplicand mod 65536) +
       (65536 * (i div 65536 * (multiplicand mod 65536)) +
         65536 * (multiplicand div 65536 * (i mod 65536))))) mod
           4294967296 = (multiplicand + i * multiplicand) mod 4294967296"
    using [[linarith_split_limit=100]] mod_add_cong
      by (simp add: algebra_simps, blast)
  thus ?case
  proof (cases "2^16 dvd (i + 1)")
    case True
    thus ?thesis by (simp add: safe_div,
                     subst mult.commute,
                     subst doublet_pow,
                     subst quadlet_pow2,
                     subst pow_div,
                     simp_all add: `0 <= multiplicand`)
  next
    case False
    thus ?thesis
      by (auto simp add: `0 <= i` IH mod1 div_truncate_one algebra_simps)
  qed
qed
  thus ?thesis using [[linarith_split_limit=100]]
(* Converting this already CPU-intensive section into an ISAR "by" block *
 * caused severe performance degradation. Therefore, it remains as a     *
 * tactics script that is best understood by running it interactively.   *)
  apply (subst `
    modular_sum
      (left_shift
        (modular_sum
          (right_shift multiplier 16 *
            (multiplicand AND 65535))
            ((multiplier AND 65535) *
        ...
        (multiplicand AND 65535)) =
          (left_shift
            (modular_sum
              ...
              ((multiplier AND 65535) *
                 right_shift multiplicand 16))
           16 + (multiplier AND 65535) *
           (multiplicand AND 65535)
          ) mod 4294967296`
  )
  apply (subst `
  left_shift
    (modular_sum (right_shift multiplier 16 *
      (multiplicand AND 65535))
        ((multiplier AND 65535) * right_shift multiplicand 16)) 16 =
  modular_sum (right_shift multiplier 16 * (multiplicand AND 65535))
    ((multiplier AND 65535) * right_shift multiplicand 16) *
      65536 mod 4294967296`
  )
  apply (subst `
    modular_sum
      (right_shift multiplier 16 * (multiplicand AND 65535))
      ((multiplier AND 65535) * right_shift multiplicand 16) =
         (right_shift multiplier 16 *
           (multiplicand AND 65535) +
           (multiplier AND 65535) *
             right_shift multiplicand 16) mod 4294967296`
  )
  apply (subst `right_shift multiplicand 16 =
                multiplicand sdiv 65536`)
  apply (subst `right_shift multiplier 16 = multiplier sdiv 65536`)
  apply (subst AND_mod16)+
  apply (simp add: `0 <= multiplier` `0 <= multiplicand` sdiv_pos_pos
                   mod_simps algebra_simps)
  apply (insert `0 <= multiplier`)
  done
qed
